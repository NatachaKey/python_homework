...
#TO CLEAN DUPLICATES, WORKS 1 TIME WITH PYTHON X.PY EXECUTION
cursor.execute("""
WITH CTE AS (
SELECT enrollment_id,
ROW_NUMBER() OVER (PARTITION BY student_id, course_id ORDER BY enrollment_id) AS row_num
FROM Enrollments
)
DELETE FROM Enrollments
WHERE enrollment_id IN (
SELECT enrollment_id FROM CTE WHERE row_num > 1
)
""")
cursor.execute("""
CREATE UNIQUE INDEX IF NOT EXISTS unique_enrollment 
